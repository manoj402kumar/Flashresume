...
def _extract_text(response) -> str | None:
    """Safely pull text from Cerebras response. Returns None on any structural issue."""
    try:
...
def _call_cerebras_single(model: str, prompt: str, max_tokens: int, retries: int = 1) -> dict:
    attempts = []
...
def _call_cerebras_chain(prompt: str, chain: list, max_tokens: int) -> dict:
    attempts = []
    for model in chain:
        result = _call_cerebras_single(model, prompt, max_tokens)
...
def call_cerebras_r2(prompt: str) -> dict:
    """Cerebras chain for Request-2 — resume generation."""
    return _call_cerebras_chain(prompt, CEREBRAS_R2_CHAIN, max_tokens=3500)


def call_single_cerebras(model: str, prompt: str, max_tokens: int = 3500) -> dict:
    """Call exactly one Cerebras model. Used by master flat chain."""
    return _call_cerebras_single(model, prompt, max_tokens)
