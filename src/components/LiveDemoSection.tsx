      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-[1fr_1.6fr] gap-8 sm:gap-12 lg:items-start">
                  ...
                  >
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${isActive ? "bg-primary/15" : "bg-surface-container-high"}`}>
                      {isDone
                        ...
                        : <Icon className={`w-4 h-4 ${isActive ? step.color : "text-on-surface-variant"}`} />
                      }
                    </div>
                    <span className={`text-[10px] font-bold text-center leading-tight ${isActive ? "text-on-background" : "text-on-surface-variant"}`}>
                      {step.label}
                    </span>
                  </button>
                );
                  ...
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${isActive ? "bg-primary/15" : isDone ? "bg-primary/5" : "bg-surface-container-high"}`}>
                      {isDone ? (
                        ...
                        <Icon className={`w-5 h-5 ${isActive ? step.color : "text-on-surface-variant"}`} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`font-bold text-sm ${isActive ? "text-on-background" : "text-on-surface-variant"}`}>
                        Step {i + 1}
                      </p>
                      <p className={`font-headline text-lg font-bold ${isActive ? "text-on-background" : "text-on-surface-variant/60"}`}>
                        {step.label}
                      </p>
                    </div>
                    {isActive && <motion.div layoutId="active-pip" className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />}
                  </motion.button>
                );
              })}
            </div>
          ...
          >
            ...
            {/* Floating badge — clipped to prevent overflow on mobile */}
            <motion.div
              initial={{ opacity: 0, scale: 0.8, y: 10 }}
              whileInView={{ opacity: 1, scale: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.8 }}
              className="absolute -bottom-4 right-2 sm:-bottom-5 sm:-right-5 flash-gradient text-white text-[10px] sm:text-xs font-black px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl shadow-xl shadow-primary/30 flex items-center gap-1.5"
            >
